...
std::vector<std::string> ArgumentVector::argv() {
    ...
    for(auto const&argument : arguments){
        argv.push_back(as_string(argument));        
    }
    return argv;
}
...
ArgumentVector& ArgumentVector::add_argument(std::string flag, bool one_flag_only , std::vector<std::string> values){
    if(!std::any_of(arguments.begin(),arguments.end(),[&flag](const Argument& arg){
